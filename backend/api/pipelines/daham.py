# backend/api/pipelines/daham.py
...
ADVERTISER_NAME = "Daham"
S3_CSV_KEY = "pipeline-data/daham.csv"
...
def run(date_from: date, date_to: date):
    ...
    print(f"🚀 Running Daham pipeline {date_from} → {date_to}")
    ...
    store_raw_snapshot(advertiser, raw_df, date_from, date_to, source="daham_csv_sheet")

    # 3. CLEAN
    clean_df = clean_daham(raw_df, advertiser)
    print("🔍 CLEAN DF HEAD:")
    ...
    push_daham_to_performance(date_from, date_to)

    print(f"✅ Daham pipeline inserted {count} rows.")
    return count
...
def fetch_raw_data() -> pd.DataFrame:
    print("📄 Loading Daham CSV from S3...")
...
def clean_daham(df: pd.DataFrame, advertiser: Advertiser) -> pd.DataFrame:
    """Clean CSV data for Daham."""
    df = df.copy()
...
def save_final_rows(advertiser: Advertiser, df: pd.DataFrame, date_from: date, date_to: date) -> int:
    if df.empty:
        DahamTransaction.objects.filter(
    ...
    with transaction.atomic():
        ...
        DahamTransaction.objects.bulk_create(objs, batch_size=500)
...
def push_daham_to_performance(date_from, date_to):
    advertiser = Advertiser.objects.filter(name=ADVERTISER_NAME).first()
    qs = DahamTransaction.objects.filter(
        order_date__gte=date_from,
        order_date__lte=date_to
    )

    if not qs.exists():
        print("⚠️ No Daham transactions found")
        return 0
    ...
    print(f"✅ Aggregated {len(objs)} Daham performance rows.")
    return len(objs)
